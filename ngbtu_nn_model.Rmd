...
p1 <- ggplot(hyper_summary_best, 
  ...
  geom_line() +
  geom_line(aes(x=num_vars, y=sqrt(mse)/(max(sqrt(hyper_summary_best$mse))*3), color='RMSE')) +
  scale_y_continuous(sec.axis = sec_axis(~.*max(sqrt(hyper_summary_best$mse)*3), name="Root Mean Square Error", breaks = seq(30000, 33000, 1000)), breaks = seq(0.7, 4.0, 0.5)) +
...
ggsave(paste0('Documents/Images/natural_gas_nn_error.png'), p1, width = 6.5, height = 3.5, units = 'in')
p1
...
model <- s3read_using(load_model_hdf5, 
                      object = 'ModelSaves/ngbtu_nn_full_model.h5', 
...
model %>% summary()
s3load('ModelSaves/ngbtu_nn_full_model_history.RData', bucket = 'cuny-msds-final-project-cbecs')
```

#Predict and examine residuals
```{r fig.width=10}
png(paste0('Documents/Images/natural_gas_nn_full_train_results.png'), width = 7, height = 4, 
...
summary(resid_model)
png(paste0('Documents/Images/natural_gas_nn_full_resid_1.png'), width = 7, height = 4, 
      units = 'in', pointsize = 12,
      bg='white', res=100)
par(mfrow=c(1,2))
plot(resid_model, c(1,2))
dev.off()
png(paste0('Documents/Images/natural_gas_nn_full_resid_2.png'), width = 7, height = 4, 
      units = 'in', pointsize = 12,
      bg='white', res=100)
par(mfrow=c(1,2))
...
p1 <- ggplot(data = data.frame(pred=model %>% predict(as.matrix(validation_reduced_df)),
    ...
    labs(x='Predicted mmBTU', y="Observed mmBTU") +
...
p2 <- ggplot(resid_df, aes(x=ngbtu, y=resid)) + 
    ...
    labs(x='Predicted mmBTU', y="Residuals") +
    ...
    theme(axis.text.x = element_text(angle=60, hjust=1),
          ...
          legend.key.height = unit(0.5, "cm"),
          title = element_text(size=20, color = "grey55"))
ggsave(paste0('Documents/Images/natural_gas_nn_full_pvr.png'), p2, width = 6.5, height = 3, units = 'in')
p2
...
matrix(append(variable_df$Names, variable_df$Label), ncol = 2, byrow = FALSE)[1:25,],
