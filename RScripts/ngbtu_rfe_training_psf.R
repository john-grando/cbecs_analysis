...
rfe_train <- rfe(y = cbecs_ngbtu_encoded_center_scale_train_df$NGBTUPerSf,
                 x = cbecs_ngbtu_encoded_center_scale_train_df %>% select(-NGBTUPerSf),
                 sizes = c(seq(50, 700, 50)),
                 rfeControl = ctrl)

#save model
model_name <- 'ModelSaves/ngbtu_rfe_psf.RData'
