...
source('RScripts/ngbtu_nn_base_model.R')
#Get model types
source('RScripts/elbtu_nn_model_functions_build.R')

#if hyperparameter df exists then use it
if(head_object('ModelSaves/ngbtu_nn_hyperparameter_results.RData', bucket='cuny-msds-final-project-cbecs')[1]==TRUE){
  s3load('ModelSaves/ngbtu_nn_hyperparameter_results.RData', bucket = 'cuny-msds-final-project-cbecs')
}
if(head_object('ModelSaves/ngbtu_nn_hyperparameter_results.RData', bucket='cuny-msds-final-project-cbecs')[1]==FALSE){
...
for(v in c(seq(0,length(variables_by_importance),300), 
  ...
  model_name <- 'ModelSaves/ngbtu_nn_hyperparameter_results.RData'
