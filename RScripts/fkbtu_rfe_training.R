...
rfe_train <- rfe(y = cbecs_fkbtu_encoded_center_scale_train_df$FKBTU,
                 x = cbecs_fkbtu_encoded_center_scale_train_df %>% select(-FKBTU),
                 sizes = c(seq(50, 700, 50)),
                 rfeControl = ctrl)

#save model
model_name <- 'ModelSaves/fkbtu_rfe.RData'
