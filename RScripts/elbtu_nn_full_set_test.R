...
if(head_object('ModelSaves/elbtu_nn_full_set_test_results_psf.RData', bucket='cuny-msds-final-project-cbecs')[1]==TRUE){
  s3load('ModelSaves/elbtu_nn_full_set_test_results_psf.RData', bucket = 'cuny-msds-final-project-cbecs')
}
if(head_object('ModelSaves/elbtu_nn_full_set_test_results_psf.RData', bucket='cuny-msds-final-project-cbecs')[1]==FALSE){
...
model_run <- function(model_n_f, n_dropout_f, n_units_f, n_l_f, loss_func_f, optimizer_func_f, batch_size_f, num_vars_f) {
  ...
  model_name <- 'ModelSaves/elbtu_nn_full_set_test_results_psf.RData'
  tmp_df <- data.frame(
...
#Make list of models to run
  run_list <- list(list(m_n='4', 
                   ...
                   list(m_n='5', 
                        ...
                        n_l=0, 
                        l_f=list(name = 'custom_loss', func = keras::loss_mean_squared_logarithmic_error), 
                        o_f=list(name = 'rmsprop_001', func = keras::optimizer_rmsprop(lr=0.005)),
                        b_f=150,
                        n_v=100),
                   list(m_n='5', 
                        n_d=0.3, 
