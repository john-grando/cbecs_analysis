...
cbecs_fkbtu_encoded_df <- tmp_df %>% 
  ...
  bind_cols(FKBTU = cbecs_fk_encoded_df %>% select(FKBTU))
...
cbecs_fkbtu_encoded_offset_df <- cbecs_fkbtu_encoded_df %>% 
  mutate_at(vars(cbecs_fkbtu_encoded_numerics_cols), funs(. + 0.5 * min(.[which(. > 0)])))
...
cbecs_fkbtu_encoded_center_scale_df <- predict(fkbtu_pre_process, 
                                               cbecs_fkbtu_encoded_df %>% 
                                                 mutate_at(vars(cbecs_fkbtu_encoded_numerics_cols), 
