---
title: "cbecs_district_heat_analysis"
author: "John Grando"
...
Response Variable
```{r}
ggplot(cbecs_dh_cleaned_df, aes(x=DHBTUPerSf)) + 
  geom_density()
#lmbda <- BoxCoxTrans(cbecs_dh_cleaned_df$DHBTU + 1)$lambda
lmbda <- 0
if(lmbda!=0){
  ggplot(cbecs_dh_cleaned_df, aes(x=DHBTUPerSf^lmbda)) + 
    geom_density() + 
    stat_function(fun=dnorm,
                color="blue",
                args=list(
                  mean=mean(cbecs_dfs$clean_df$DHBTUPerSf^lmbda, na.rm = TRUE),
                  sd(cbecs_dfs$clean_df$DHBTUPerSf^lmbda, na.rm = TRUE)
                )
    ) +
...
if(lmbda==0){
  ggplot(cbecs_dh_cleaned_df, aes(x=log(DHBTUPerSf+1))) + 
    geom_density() + 
    stat_function(fun=dnorm,
                color="blue",
                args=list(
                  mean=mean(log(cbecs_dfs$clean_df$DHBTUPerSf+1), na.rm = TRUE),
                  sd(log(cbecs_dfs$clean_df$DHBTUPerSf+1), na.rm = TRUE)
                )
    ) +
    ggtitle(paste('DHBTUPerSf Transformed via BoxCox, Lambda = ', lmbda, sep="")) +
    theme(axis.text.x = element_text(angle=60, hjust=1),
          plot.title = element_text(hjust = 0.5,  size=16),
          plot.background = element_rect(fill = "lightgrey"), 
          panel.background = element_rect(fill = "white"), 
...
ggsave('Documents/Images/district_heat_psf_response.png', width = 6.5, height = 2, units = 'in')
...
```{r}
set.seed(20)
s3load('ModelSaves/dhbtu_pca_psf.RData', bucket = 'cuny-msds-final-project-cbecs')
...
ggsave('Documents/Images/district_heat_psf_pca_vars.png', g, width = 7.5, height = 5, units = 'in')
```

#PLS 
```{r}
set.seed(20)
s3load('ModelSaves/dhbtu_pls_psf.RData', bucket = 'cuny-msds-final-project-cbecs')
print(pls_train)
varImp(pls_train)
pls_plot <- plot_fun(in_model=pls_train, 
                     ...
                     response = cbecs_dhbtu_encoded_center_scale_train_df$DHBTUPerSf,
                     fuel_type = 'district_heat_psf')
pls_plot$vars_plot
pls_plot$pvo_plot
...
```{r fig.height=10}
set.seed(20)
s3load('ModelSaves/dhbtu_rf_psf.RData', bucket = 'cuny-msds-final-project-cbecs')
print(rf_train)
varImp(rf_train)
...
```{r eval=FALSE}
set.seed(20)
s3load('ModelSaves/dhbtu_l_psf.RData', bucket = 'cuny-msds-final-project-cbecs')
print(l_train)
varImp(l_train)
...
```{r}
set.seed(20)
s3load('ModelSaves/dhbtu_lp_psf.RData', bucket = 'cuny-msds-final-project-cbecs')
print(lp_train)
varImp(lp_train)
...
```{r}
set.seed(20)
s3load('ModelSaves/dhbtu_rfe_psf.RData', bucket = 'cuny-msds-final-project-cbecs')
rfe_train
...
rfe_plot <- plot_pred_obs(in_model_sub = rfe_train, 
                          ...
                          observed_df = cbecs_dhbtu_encoded_center_scale_train_df,
                          response = cbecs_dhbtu_encoded_center_scale_train_df$DHBTUPerSf,
                          fuel_type_sub = 'district_heat_psf')
...
set.seed(20)
s3load('ModelSaves/dhbtu_nn_psf.RData', bucket = 'cuny-msds-final-project-cbecs')
print(nn_train)
varImp(nn_train)
...
```{r eval=FALSE}
s3load('ModelSaves/dhbtu_pca.RData', bucket = 'cuny-msds-final-project')
s3load('ModelSaves/dhbtu_pls.RData', bucket = 'cuny-msds-final-project')
s3load('ModelSaves/dhbtu_rf.RData', bucket = 'cuny-msds-final-project')
s3load('ModelSaves/dhbtu_l.RData', bucket = 'cuny-msds-final-project')
s3load('ModelSaves/dhbtu_lp.RData', bucket = 'cuny-msds-final-project')
s3load('ModelSaves/dhbtu_rfe.RData', bucket = 'cuny-msds-final-project')
s3load('ModelSaves/dhbtu_nn.RData', bucket = 'cuny-msds-final-project')
```
...
sqft_values <- cbecs_dh_cleaned_df$SQFT

nn_input_data_name <- 'ModelSaves/dhbtu_nn_input_psf.RData'
...
p_all_v <- ggplot(data = variable_by_importance_df %>% head(20), 
    ...
    theme(axis.text.x = element_text(angle=60, hjust=1),
          ...
          legend.key.height = unit(0.5, "cm"),
          title = element_text(size=20, color = "grey55"))
ggsave(paste0('Documents/Images/district_heat_psf_all_vars.png'), p_all_v, width = 6.5, height = 3.5, units = 'in')
p_all_v
```
