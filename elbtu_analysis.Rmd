---
title: "cbecs_electricity_analysis"
author: "John Grando"
...
Response Variable
```{r}
ggplot(cbecs_fk_cleaned_df, aes(x=ELBTU)) + 
  geom_density()
#lmbda <- BoxCoxTrans(cbecs_fk_cleaned_df$ELBTU + 1)$lambda
lmbda <- 0
if(lmbda!=0){
  ggplot(cbecs_fk_cleaned_df, aes(x=ELBTU^lmbda)) + 
    geom_density() + 
...
if(lmbda==0){
  ggplot(cbecs_fk_cleaned_df, aes(x=log(ELBTU+1))) + 
    geom_density() + 
    stat_function(fun=dnorm,
                color="blue",
                args=list(
                  mean=mean(log(cbecs_dfs$clean_df$ELBTU+1), na.rm = TRUE),
                  sd(log(cbecs_dfs$clean_df$ELBTU+1), na.rm = TRUE)
                )
    ) +
    ggtitle(paste('ELBTU Transformed via BoxCox, Lambda = ', lmbda, sep="")) +
    theme(axis.text.x = element_text(angle=60, hjust=1),
...
```{r}
set.seed(20)
s3load('ModelSaves/elbtu_pca.RData', bucket = 'cuny-msds-final-project-cbecs')
...
```{r}
set.seed(20)
s3load('ModelSaves/elbtu_pls.RData', bucket = 'cuny-msds-final-project-cbecs')
print(pls_train)
varImp(pls_train)
...
pls_plot$RMSE
pls_plot$R2
#View(t(cbecs_fk_cleaned_df %>% tibble::rownames_to_column() %>% mutate(pred = exp(predict(pls_train, newdata = cbecs_elbtu_encoded_center_scale_df))) %>% filter(rowname==406|rowname==4974) %>% select(ELBTU, pred, SQFTC, RFGVEN, RFGICE, RFGWI, EMCS, GENUSE, one_of(pls_imp_df$feature))))
...
```{r fig.height=10}
set.seed(20)
s3load('ModelSaves/elbtu_rf.RData', bucket = 'cuny-msds-final-project-cbecs')
print(rf_train)
varImp(rf_train)
...
#View(t(cbecs_fk_cleaned_df %>% tibble::rownames_to_column() %>% mutate(pred = predict(rf_train, newdata = cbecs_elbtu_encoded_center_scale_df)) %>% filter(rowname==507|rowname==406|rowname==4974)))
...
```{r eval=FALSE}
set.seed(20)
s3load('ModelSaves/elbtu_l.RData', bucket = 'cuny-msds-final-project-cbecs')
print(l_train)
varImp(l_train)
...
```{r}
set.seed(20)
s3load('ModelSaves/elbtu_lp.RData', bucket = 'cuny-msds-final-project-cbecs')
print(lp_train)
varImp(lp_train)
...
```{r}
set.seed(20)
s3load('ModelSaves/elbtu_rfe.RData', bucket = 'cuny-msds-final-project-cbecs')
rfe_train
...
set.seed(20)
s3load('ModelSaves/elbtu_nn.RData', bucket = 'cuny-msds-final-project-cbecs')
print(nn_train)
varImp(nn_train)
