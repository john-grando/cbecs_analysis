...
#ploting funcitons
```{r}
plot_fun <- function(in_model=NA, n_features=2, short_name=NA, alt_predict=FALSE, log_tran=FALSE, observed_df=NA) {
  ...
  ggsave(paste0('Documents/Images/electricity_',short_name,'_vars.png'), p1, width = 6.5, height = 3.5, units = 'in')
  imp_df <- plot_df %>% 
    select(feature)
...
plot_pred_obs <- function(in_model_sub = NA, short_name_sub = short_name, alt_predict_sub=FALSE, log_tran_sub=FALSE, observed_df_sub=NA){
  ...
  p2 <- p2 + 
    ...
    theme(axis.text.x = element_text(angle=60, hjust=1),
          ...
          legend.key.height = unit(0.5, "cm"),
          title = element_text(size=20, color = "grey55"))
  ggsave(paste0('Documents/Images/electricity_',short_name_sub,'_pvo.png'), p2, width = 6.5, height = 3, units = 'in')
  png(paste0('Documents/Images/electricity_',short_name_sub,'_res_1.png'), width = 7, height = 4, 
      units = 'in', pointsize = 12,
      bg='white', res=100)
  par(mfrow=c(1,2))
  plot(tmp_model_sub, c(1,2))
  dev.off()
  png(paste0('Documents/Images/electricity_',short_name_sub,'_res_2.png'), width = 7, height = 4, 
      units = 'in', pointsize = 12,
      bg='white', res=100)
  par(mfrow=c(1,2))
...
ggsave('Documents/Images/electricity_response.png', width = 6.5, height = 2, units = 'in')
...
ggsave('Documents/Images/electricity_pca_vars.png', g, width = 7.5, height = 5, units = 'in')
```
...
rfe_imp_df <- rfe_train$variables %>% filter(Variables == top_n_rows) %>% select(var) %>% distinct() %>% rename(feature = var)
...
```{r}
plot_vars <- function(df, response, response_char, i, file_index, trans_name){
  pre_p <- ggplot(df) #+
    #theme(axis.title = element_text(size=2))
  if(length(unique(df[,i])) > 15){
    tmp_p <- pre_p + aes(x=df[,i], y=log(response)) +
      geom_point() +
      labs(x=i, y=paste(response_char, 'MBTU Logscale', sep=" ")) +
    theme(axis.text.x = element_text(angle=60, hjust=1),
          plot.title = element_text(hjust = 0.5,  size=16),
          plot.background = element_rect(fill = "lightgrey"), 
          panel.background = element_rect(fill = "white"), 
  ...
  if(length(unique(df[,i])) <= 15){
    tmp_p <- pre_p +  aes(x=as.factor(df[,i]), y=log(response)) +
      geom_violin() +
      labs(x=i, y=paste(response_char, 'MBTU Logscale', sep=" ")) +
    theme(axis.text.x = element_text(angle=60, hjust=1),
          plot.title = element_text(hjust = 0.5,  size=16),
...
```{r}
tst <- colnames(nn_input_df %>% select(-ELBTU))
iter <- 0
for(i in tst){
...
for(i in tst){
  plot_vars(cbecs_elbtu_encoded_center_scale_df, 
            cbecs_elbtu_encoded_center_scale_df$ELBTU, 
            'electricity', 
            i, 
            iter,
            'transformed')
