...
plot_fun <- function(in_model=NA, n_features=2, short_name=NA, alt_predict=FALSE, log_tran=FALSE) {
  ...
  if(alt_predict==FALSE){
    final_model <- in_model$finalModel
  }
  if(alt_predict!=FALSE){
  ...
  if(log_tran==TRUE){
    tmp_model <- lm(exp(predict(final_model, cbecs_elbtu_encoded_center_scale_train_df)) ~ 
      cbecs_elbtu_encoded_center_scale_train_df$ELBTU)
  }
  if(log_tran!=TRUE){
    tmp_model <- lm(predict(final_model, cbecs_elbtu_encoded_center_scale_train_df) ~ 
      cbecs_elbtu_encoded_center_scale_train_df$ELBTU)
  }
  p2 <- ggplot(data = data.frame(predicted = tmp_model$fitted.values, 
                                 observed = cbecs_elbtu_encoded_center_scale_train_df$ELBTU), 
    ...
    theme(axis.text.x = element_text(angle=60, hjust=1),
          ...
          legend.key.height = unit(0.5, "cm"),
          title = element_text(size=20, color = "grey55"))
  ggsave(paste0('Documents/Images/electricity_',short_name,'_pvo.png'), p2, width = 6.5, height = 3, units = 'in')
  png(paste0('Documents/Images/electricity_',short_name,'_res_1.png'), width = 7, height = 4, 
      units = 'in', pointsize = 12,
      bg='white', res=100)
  par(mfrow=c(1,2))
  plot(tmp_model, c(1,2))
  png(paste0('Documents/Images/electricity_',short_name,'_res_2.png'), width = 7, height = 4, 
      units = 'in', pointsize = 12,
      bg='white', res=100)
  par(mfrow=c(1,2))
  plot(tmp_model, c(3,5))
  dev.off()
  return(list(vars_plot = p1, 
              pvo_plot = p2,
              tmp_model = tmp_model,
              imp_df = imp_df,
              RMSE = RMSE(cbecs_elbtu_encoded_center_scale_train_df$ELBTU, pls_plot$tmp_model$fitted.values))
  )
}
```
...
pls_imp_df <- pls_plot$imp_dfp_df
pls_plot$RMSE
