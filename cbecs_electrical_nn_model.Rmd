...
#Build keras model
```{r}
build_model <- function(df = NA, n_dropout=0, n_units=10, n_l = 0) {
...
hyper_list <- list()
hyper_list$dropout <- seq(0.1, 0.5, 0.3)
hyper_list$units <- seq(100, 600, 250)
  ...
  for (d in hyper_list$dropout) {
    for (u in hyper_list$units) {
      tmp_df <- foreach(f = 1:n_folds,
        ...
        cv_model_t <- build_model(df = train_df, n_dropout = d, n_units = u, n_l=r)
        ...
        data.frame(dropout=d, units=u, reg=r, fold=f, loss= loss_val, mae = mae_val, pm = pm_val)
      }
    print(paste('finished running model: droput, units, regularizer', d, u, r, sep = " "))
