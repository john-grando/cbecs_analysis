...
custom_loss_func <- function(y_true, y_pred) {
  ...
  K$mean(tf$multiply(K$square(y_pred - y_true) / K$clip(y_true,0.1,700), second_y))
}

percentage_metric <- custom_metric('percentage_metric', function(y_true, y_pred){
  K <- backend()
  K$mean(tf$multiply(K$abs(1 - K$abs(y_true - y_pred) / K$abs(y_true)), K$relu(y_true)))
})
```
...
resid_df <- data.frame(resid = residuals, 
                       ...
                       pred = model %>% predict(validation_df),
                       ratio = abs(1 - model %>% predict(validation_df) / validation_labels))
