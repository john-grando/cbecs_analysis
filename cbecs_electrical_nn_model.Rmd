...
```{r}
folds <- createFolds(y = train_test_list, k=5, list=FALSE)

hyper_list <- list()
hyper_list$dropout <- seq(0.1, 0.5, 0.2)
hyper_list$units <- seq(400, 800, 200)
```
...
for (d in hyper_list$dropout) {
  for (u in hyper_list$units) {
    for (f in unique(folds)){
      ...
      print(paste('finished running model, mae, pm:',mae_val, pm_val, sep=' '))
    }
  }
}
