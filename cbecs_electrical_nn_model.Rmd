...
#Build keras model
```{r}
build_model <- function(df = NA, n_dropout=0, n_units=10) {
...
#                kernel_regularizer = regularizer_l2()
  ...
  model %>% compile(
    ...
    optimizer = keras::optimizer_sgd(lr = 0.02, decay = 0.05),
...
hyper_list <- list()
hyper_list$dropout <- seq(0.5, 0.5, 0.2)
hyper_list$units <- seq(10, 10, 200)
```
...
for (d in hyper_list$dropout) {
  for (u in hyper_list$units) {
    tmp_df <- foreach(f = 1:n_folds,
                   .combine = rbind) %dopar% {
      print(paste('running dropout, units, fold:',d,u,f, sep=' '))
      ind <- which(folds == f)
      ...
      cv_model_t <- build_model(df = train_df, n_dropout = d, n_units = u)
      ...
      data.frame(dropout=d, units=u, fold=f, loss= loss_val, mae = mae_val, pm = pm_val)
    }
  print(paste('finished running model: droput, units', d, u, sep = " "))
...
model <- build_model(df=train_df, n_dropout=0.2, n_units = 100)
model %>% summary()
