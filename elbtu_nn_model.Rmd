...
p1 <- ggplot(hyper_summary_best, 
       aes(x=num_vars, y=msle)) + 
  geom_line() +
  ggtitle('Loss vs Number of Variables') +
  labs(x='Number of Variables', y='Mean Squared Loss Error') +
...
ggsave(paste0('Documents/Images/electricity_nn_error.png'), p1, width = 6.5, height = 3.5, units = 'in')
p1
...
model %>% summary()
s3load('ModelSaves/elbtu_nn_full_model_history.RData', bucket = 'cuny-msds-final-project-cbecs')
```

#Predict and examine residuals
```{r}
png(paste0('Documents/Images/electricity_nn_full_train_results.png'), width = 7, height = 4, 
      units = 'in', pointsize = 12,
      bg='white', res=100)
plot(history) + theme(strip.text.y = element_text(size = 7))
dev.off()
...
summary(resid_model)
png(paste0('Documents/Images/electricity_nn_full_resid_1.png'), width = 7, height = 4, 
      units = 'in', pointsize = 12,
      bg='white', res=100)
par(mfrow=c(1,2))
plot(resid_model, c(1,2))
dev.off()
png(paste0('Documents/Images/electricity_nn_full_resid_2.png'), width = 7, height = 4, 
      units = 'in', pointsize = 12,
      bg='white', res=100)
par(mfrow=c(1,2))
...
p2 <- ggplot(resid_df, aes(x=elbtu, y=resid)) + 
    ...
    theme(axis.text.x = element_text(angle=60, hjust=1),
          ...
          legend.key.height = unit(0.5, "cm"),
          title = element_text(size=20, color = "grey55"))
ggsave(paste0('Documents/Images/electricity_nn_full_pvr.png'), p2, width = 6.5, height = 3, units = 'in')
p2
...
nn_rmse <- caret::RMSE(pred = resid_df$pred, obs = resid_df$elbtu)
nn_r2 <- caret::R2(pred = resid_df$pred, obs = resid_df$elbtu)
nn_mae <- caret::MAE(pred = resid_df$pred, obs = resid_df$elbtu)
