...
hyper_summary <- hyper_results %>% 
  group_by(loss, opt, model, dropout, units, reg, batch) %>% 
...
p1 <- ggplot(data = data.frame(pred=model %>% predict(as.matrix(validation_df)),
...
p1
p2 <- ggplot(resid_df, aes(x=elbtu_per_sf, y=resid)) + 
    geom_point(alpha=0.2) +
...
elbtu_impact_error_check_df <- resid_df %>% 
  group_by(pba) %>% 
  summarize(avg_elbtu = mean(elbtu_per_sf),
            avg_pred_elbtu = mean(pred),
            RMSE = caret::RMSE(pred = pred, obs = elbtu_per_sf),
            RMSE_ratio = RMSE / avg_elbtu,
            R2 = caret::R2(pred=pred,obs=elbtu_per_sf),
            instances = n())

caret::RMSE(pred = resid_df$pred, obs = resid_df$elbtu_per_sf)
caret::R2(pred = resid_df$pred, obs = resid_df$elbtu_per_sf)
caret::MAE(pred = resid_df$pred, obs = resid_df$elbtu_per_sf)
```
