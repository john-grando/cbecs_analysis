...
#k means
```{r fig.height=10}
kmeans_fit <- kmeans(cbecs_clean_scaled_df, centers=4, nstart = 25)
fviz_cluster(kmeans_fit, data=cbecs_clean_scaled_df)
ggplot(data=cbind(cbecs_clean_df, kgroup = kmeans_fit$cluster), aes(x=log(MFBTUPerSf/WKHRS), y=log(SERVERNPerSf), color=as.factor(kgroup), shape=as.factor(kgroup))) + geom_point(aes(alpha=0.4)) + facet_wrap(~DHUSED+factor(ifelse(RFGWINPerSf>0|RFGCLNPerSf>0|RGSTRNPerSf,1,0)), scales = 'free')
```


#PCA
```{r fig.width=14}
cbecs_pca <- prcomp(cbecs_clean_df[, cbecs_clean_numeric_cols], 
                    center = TRUE, 
                    scale. = TRUE)
screeplot(cbecs_pca, type = 'lines')
summary(cbecs_pca)
ggplot(cbecs_pca$x, aes(x=PC1, y=PC2, color=cbecs_clean_df$PBA)) + geom_point()
