...
#Remove old postings
time_summary <- hyper_results %>% 
  #change similar num_vars to be consistent
  mutate(num_vars = replace(num_vars, num_vars==516, 500)) %>% 
  group_by(
    num_vars, 
    loss_func, 
    opt, 
...
summary_df <- manual_summary %>% 
  ...
  MAPE=mean(abs(resid_df$pred - resid_df$elbtu) / pmax(resid_df$elbtu, 1)) * 100
))
...
summary_df <- manual_summary %>% 
  ...
  MAPE=mean(abs(resid_df$pred / 1000000 - resid_df$elbtu / 1000000) / pmax(resid_df$elbtu / 1000000, 1)) * 100
))
