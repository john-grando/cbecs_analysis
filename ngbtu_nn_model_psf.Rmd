...
p1 <- ggplot(hyper_summary_best, 
  ...
  geom_line() +
  geom_line(aes(x=num_vars, y=sqrt(mse)/max(sqrt(mse))*1.5, color='RMSE')) +
  scale_y_continuous(sec.axis = sec_axis(~.*max(sqrt(hyper_summary_best$mse))/1.5, name="Root Mean Square Error", breaks = seq(70000, 85000, 5000)), breaks = seq(1.2, 1.6, 0.1)) +
...
ggsave(paste0('Documents/Images/natural_gas_nn_full_pvo.png'), p1, width = 6.5, height = 3, units = 'in')
p1
...
p1 <- ggplot(data = data.frame(pred=model %>% predict(as.matrix(validation_reduced_df)) * validation_sqft_values /1000000,
    ...
    geom_point(alpha=0.2) +
    ggtitle('Predicted vs. Observed') +
    labs(x='Predicted mmBTU', y='Observed BTU Per SF') +
    geom_abline(slope = 1, intercept = 0) +
    theme(axis.text.x = element_text(angle=60, hjust=1),
          plot.title = element_text(hjust = 0.5),
          plot.background = element_rect(fill = "lightgrey"), 
          ...
          legend.key.height = unit(0.5, "cm"),
          title = element_text(size=20, color = "grey55"))
ggsave(paste0('Documents/Images/natural_gas_nn_full_pvo_transformed.png'), p1, width = 6.5, height = 3, units = 'in')
p1
```
...
variable_df <- data.frame(Names = gsub('_bin\\.+[[:digit:]]+|\\.+[[:digit:]]+', '', colnames(validation_reduced_df))) %>% 
