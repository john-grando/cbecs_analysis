...
p1 <- ggplot(hyper_summary_best, 
       aes(x=num_vars, y=mape, color='MAPE')) + 
  geom_line() +
  geom_line(aes(x=num_vars, y=sqrt(mse)/(max(sqrt(mse)/60)), color='RMSE')) +
  scale_x_continuous(breaks = seq(0, 800, 50)) +
  scale_y_continuous(sec.axis = sec_axis(~.*max(sqrt(hyper_summary_best$mse)/60), name="Root Mean Square Error", breaks = seq(20000, 100000, 10000)), breaks = seq(20, 100, 10)) +
  scale_color_manual(values = c("steelblue", "springgreen")) +
  ggtitle('Loss vs Number of Variables') +
  labs(x='Number of Variables', y='Mean Absolute Percentage Error', color='Loss Metric') +
