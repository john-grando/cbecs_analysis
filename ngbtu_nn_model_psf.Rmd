...
hyperparameter_parsed_results <- hyper_results %>% 
...
hyperparameter_summary <- hyperparameter_parsed_results %>% 
  group_by(
    num_vars, 
    loss_f, 
    opt, 
...
hyperparameter_summary_best <- hyperparameter_summary %>% 
               group_by(num_vars) %>% 
               arrange(msle) %>% 
               filter(row_number()==1)
```
...
p1 <- ggplot(hyper_summary_best, 
  ...
  geom_line() +
  geom_line(aes(x=num_vars, y=sqrt(mse)/(max(sqrt(mse))/2), color='RMSE')) +
  scale_x_continuous(breaks = seq(0, 800, 50)) +
  scale_y_continuous(sec.axis = sec_axis(~.*max(sqrt(hyper_summary_best$mse))/2, name="Root Mean Square Error", breaks = seq(60000, 90000, 5000)), breaks = seq(1.3, 1.8, 0.05)) +
  coord_cartesian(ylim = c(1.3, 1.8)) +
  scale_color_manual(values = c("steelblue", "springgreen")) +
  ggtitle('Loss vs Number of Variables') +
  labs(x='Number of Variables', y='Mean Square Log Error', color='Loss Metric') +
...
ggsave(paste0('Documents/Images/natural_gas_psf_nn_error.png'), p1, width = 6.5, height = 3.5, units = 'in')
p1
...
model <- s3read_using(load_model_hdf5, 
                      object = 'ModelSaves/ngbtu_nn_full_model_psf.h5', 
...
s3load('ModelSaves/ngbtu_nn_full_model_history_psf.RData', bucket = 'cuny-msds-final-project-cbecs')
model %>% summary()
...
```{r fig.width=10}
png(paste0('Documents/Images/natural_gas_psf_nn_full_train_results.png'), width = 7, height = 4, 
...
summary(resid_model)
png(paste0('Documents/Images/natural_gas_psf_nn_full_resid_1.png'), width = 7, height = 4, 
      units = 'in', pointsize = 12,
      bg='white', res=100)
par(mfrow=c(1,2))
plot(resid_model, c(1,2))
dev.off()
png(paste0('Documents/Images/natural_gas_psf_nn_full_resid_2.png'), width = 7, height = 4, 
      units = 'in', pointsize = 12,
      bg='white', res=100)
par(mfrow=c(1,2))
...
p1 <- ggplot(data = data.frame(pred=model %>% predict(as.matrix(validation_reduced_df)),
    ...
    theme(axis.text.x = element_text(angle=60, hjust=1),
          ...
          legend.key.height = unit(0.5, "cm"),
          title = element_text(size=14, color = "grey55"))
ggsave(paste0('Documents/Images/natural_gas_psf_nn_full_pvo.png'), p1, width = 6.5, height = 3, units = 'in')
p1
p2 <- ggplot(resid_df, aes(x=ngbtu, y=resid)) + 
    geom_point(alpha=0.2) +
    ...
    theme(axis.text.x = element_text(angle=60, hjust=1),
          ...
          legend.key.height = unit(0.5, "cm"),
          title = element_text(size=14, color = "grey55"))
ggsave(paste0('Documents/Images/natural_gas_psf_nn_full_pvr.png'), p2, width = 6.5, height = 3, units = 'in')
p2
ngbtu_impact_error_check_df <- resid_df %>% 
  group_by(pba) %>% 
  summarize(avg_ngbtu = mean(ngbtu),
            avg_pred_ngbtu = mean(pred),
            RMSE = caret::RMSE(pred = pred, obs = ngbtu),
            RMSE_ratio = RMSE / avg_ngbtu,
            R2 = caret::R2(pred=pred,obs=ngbtu),
            instances = n())

caret::RMSE(pred = resid_df$pred, obs = resid_df$ngbtu)
caret::R2(pred = resid_df$pred, obs = resid_df$ngbtu)
caret::MAE(pred = resid_df$pred, obs = resid_df$ngbtu)
```
...
summary_df <- manual_summary %>% 
  ...
  RMSE=caret::RMSE(pred = resid_df$pred, obs = resid_df$ngbtu),
  R2=caret::R2(pred = resid_df$pred, obs = resid_df$ngbtu),
  MAE=caret::MAE(pred = resid_df$pred, obs = resid_df$ngbtu),
  MAPE=mean(abs(resid_df$pred - resid_df$ngbtu) / pmax(resid_df$ngbtu, 1)) * 100,
  MSLE=mean((log(pmax(0.001,resid_df$pred)) - log(pmax(0.001,resid_df$ngbtu)))^2)
)) %>% 
  mutate(
    RMSE = round(RMSE, 0),
    R2 = round(R2, 2),
...
p <- summary_df %>% 
  arrange(MSLE) %>% 
  kable('html', 
        caption = 'Per SF Model Comparison [BTU/SF]') %>% 
  kable_styling()
p
```
...
resid_df <- data.frame(resid = residuals, 
                       pba = data.frame(pba=nn_input_pba_labels) %>% slice(-train_test_list),
                       ngbtu = validation_labels*validation_sqft_values,
...
summary_df <- manual_summary %>% 
  ...
  bind_rows(data.frame(
  Model='Full Neural Network',
  RMSE=caret::RMSE(pred = resid_df$pred / 1000000, obs = resid_df$ngbtu / 1000000),
  R2=caret::R2(pred = resid_df$pred / 1000000, obs = resid_df$ngbtu / 1000000),
  MAE=caret::MAE(pred = resid_df$pred / 1000000, obs = resid_df$ngbtu / 1000000),
  MAPE=mean(abs(resid_df$pred / 1000000 - resid_df$ngbtu / 1000000) / pmax(resid_df$ngbtu / 1000000, 1)) * 100,
  MSLE=mean((log(pmax(0,resid_df$pred / 1000000)) - log(pmax(0,resid_df$ngbtu / 1000000)))^2)
)) %>% 
  mutate(
    RMSE = round(RMSE, 0),
    R2 = round(R2, 2),
...
p <- summary_df %>% 
  arrange(MSLE) %>% 
  kable('html', 
        caption = 'Total Consumption Prediction vs. Feature Extraction Models [mmBTU]') %>% 
  kable_styling()
p
...
p1 <- ggplot(data = data.frame(pred=model %>% predict(as.matrix(validation_reduced_df)) * validation_sqft_values /1000000,
    ...
    theme(axis.text.x = element_text(angle=60, hjust=1),
          ...
          legend.key.height = unit(0.5, "cm"),
          title = element_text(size=14, color = "grey55"))
ggsave(paste0('Documents/Images/natural_gas_psf_nn_full_pvo_transformed.png'), p1, width = 11, height = 5, units = 'in')
p1
```
