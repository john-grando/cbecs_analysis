...
```{r}
ggplot(cbecs_dfs$clean_df, aes(x=COOK, y=ELBTUPerSf)) + geom_boxplot()
ggplot(cbecs_dfs$clean_df, aes(x=LAUNDR, y=ELBTUPerSf)) + geom_boxplot()
```
...
ggplot(cbecs_elbtu_encoded_center_scale_train_df, 
       aes(x=pls_train$finalModel$fitted.values[,,3]^(1/lmbda), 
...
plot(pls_train$finalModel$fitted.values[,,5]^(1/lmbda), pls_train$finalModel$residuals[,,5])
```
...
nn_input_df <- cbecs_elbtu_encoded_center_scale_df %>% 
  select(one_of(pls_imp_df$feature), 
         ...
         one_of(rf_imp_df$feature),
         one_of(colnames(cbecs_elbtu_encoded_center_scale_df %>% select(matches('^PBAPLUS'), ELBTUPerSf))),
         #Additional columns for pbaplus 49
         DAYLTP, CFLRP, DCNTRSFC..1, LNHRPC.2, CHLAIRCL.2, HWTRM.2, ENGINE.1, LTNR24.1, ELMANU.2, RFGCL.1, ANYEGY.1, ENRGYPLN.2, GENUSE.1, EMCSLT.2, NFLOORPerSf)
...
control <- trainControl(index = cbecs_train_cv_list, 
                        ...
                        repeats = 1, 
                        search = 'grid')
tunegrid <- expand.grid(size = seq(10, 210, 100), 
                        dropout = seq(0.2, 0.9, 0.7), 
                        batch_size = seq(2000, 2000, 1000), 
                        lr = seq(0.3, 0.7, 0.4),
                        decay = seq(0, 0.4, 0.2), 
                        activation = c('relu'))
nn_train <- caret::train(
  y = nn_input_train_test_df$ELBTUPerSf,
  x = nn_input_train_test_df %>% select(-ELBTUPerSf),
...
ggplot(cbecs_elbtu_encoded_center_scale_train_df, 
       aes(x=predict(nn_train, newx = predict(elbtu_pre_process, newdata = cbecs_elbtu_encoded_center_scale_train_df)),
           y=ELBTUPerSf, color = cbecs_el_cleaned_df[cbecs_train_list,c("PBAPLUS")])) + geom_point()
varImp(nn_train)
```
